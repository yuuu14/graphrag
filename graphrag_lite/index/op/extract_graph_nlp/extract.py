...
# coroutine task processing pd.Dataframe's row, find a name for this file
async def extract_graph_nlp(
    ...
    text_chunks = text_unit_df.loc[:, ["id", "text"]]
    nodes_df = await _extract_nodes(
        text_chunks, text_analyzer, num_threads=num_threads, cache=cache
    )
...
async def _extract_nodes(
    ...
    Returns a dataframe with schema [id, alias, frequency, text_unit_ids].
    """
    # TODO: change cache to storage [cache is for LLM, storage for other Run]
    ...
    noun_node_df = noun_node_df.rename(
        columns={"noun_phrases": "alias", "id": "text_unit_id"}
    )

    # group by alias and count the number of text units
    grouped_node_df = (
        noun_node_df.groupby("alias").agg({"text_unit_id": list}).reset_index()
    )
    ...
    return grouped_node_df[["alias", "frequency", "text_unit_ids"]]
...
def _extract_edges(
    ...
    Input: nodes_df with schema [id, alias, frequency, text_unit_ids]
    ...
    text_units_df = (
        ...
        .agg({"alias": lambda x: list(x) if len(x) > 1 else np.nan})
        .reset_index()
    )
    text_units_df = text_units_df.dropna()
    aliases = text_units_df["alias"].tolist()
    all_edges: list[list[tuple[str, str]]] = [list(combinations(t, 2)) for t in aliases]
